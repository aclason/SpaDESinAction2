...
1. [LandR](!PLACEHOLDER Presentation LandR!): this model is composed of 3 main modules:  
...
2. [scfm](!PLACEHOLDER Presentation LandR!) (Steve Cumming's fire model): this model is composed of 3 main modules:
  `ignition`
  `escape`
  `spread`
...
4.  [caribouRSF](!PLACEHOLDER Presentation LandR!) (resource selection function): Statistical relationships between caribou telemetry locations and habitat characteristics have been summarized (Environment Canada 2011) by resource selection function (RSF; Manly et al. 2002) type models for various parts of Canada. We used the top RSF-type model, a binomial GLMM, for the Taiga Plains ecozone (Environment Canada 2011; Table 46). This model was estimated from individual caribou location data collected from 24 Alberta, 50 British Columbia, and 169 NWT VHF-collared adult female caribou between 2000 and 2010 (Environment Canada 2011; Table 39). The analysis used a habitat use-availability design, so the model predicts relative, not absolute, resource selection (Manly et al. 2002).  
...
### 2. [Installing necessary libraries and setting options](02_librariesAndOptions.html)  
...
allModules <- c("Biomass_borealDataPrep",
                "Biomass_core",
                "scfmIgnition",
                 "scfmEscape",
                 "scfmSpread",
...
#### 8a. Generate and plot Study Area  

```{r studyArea}
polyMatrix <- matrix(c(-118.269387, 61.783558), ncol = 2)
areaSize <- 10000000
set.seed(2020)
# NWT land cover classes -- base map to locate study area
...
studyArea <- projectInputs(x = studyAreaOriginal, targetCRS = crs(regionMap))
NWT <- readRDS(file.path(getPaths()$inputPath, "NWT_boundaries.rds"))
plot(NWT); plot(regionMap, add = TRUE)
clearPlot()
Plot(regionMap, title = "The NWT"); Plot(studyArea, addTo = "regionMap", title = "The NWT")
```

#### 8b. Load objects from inputs folder
```{r fireObjects}
...
At last, we will integrate the forest succesoin, fire and caribou:
...
# Fire plots
clearPlot()
Plot(scfm$ageMap, title = "Age Map")
Plot(scfm$flammableMap, title = "Flammable Map", col = c("skyblue", "red"))
Plot(scfm$rstCurrentBurn, title = "Last year's burns", col = c("lightgrey", "red"))
Plot(scfm$burnMap, title = "Cummulative burn map")
```
