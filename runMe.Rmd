...
Require(packages = "tati-micheletti/usefun", install_githubArgs = list(ref = "master"))
...
```{r objects}
objects <- list(
  # "rasterToMatch" = RTM, # tried removing, it worked
  "vegMap" = vegetationMap,
"sppNameVector" = sppNameVector,
"studyArea" = studyArea#, #studyArea = studyArea #"studyAreaLarge" = studyAreadid not work: Error in .local(x, y, ...) : extents do not overlap
#  LCC05 = vegetationMap
...
```{r outputsLandR}
clearPlot()
plot(LandR$pixelGroupMap)
knitr::kable(rbind(head(LandR$cohortData), tail(LandR$cohortData)))
```
